...
class IPERLGoogleSheetsReader {
    companion object {
        ...
        // Water meters card: main sheet (gid=0) — column K lives on the main sheet
    ...
    // Public wrapper so callers can parse a CSV string using the same logic
    fun parseWaterMeterCsv(csvData: String, maxRows: Int = 100): List<WaterMeterReading> {
        return parseWaterMeterDataColumnAandK(csvData, maxRows)
    }
    ...
    private fun parseWaterMeterDataColumnAandK(csvData: String, maxRows: Int): List<WaterMeterReading> {
        ...
        for (i in startIdx until dataLines.size) {
            ...
            if (valueField == null || valueField.isEmpty()) {
                val numIdx = fields.indexOfLast { parseFloatNullable(it) != null || it.trim().matches(Regex("[-+]?[0-9]+(\\.[0-9]+)?")) }
                if (numIdx >= 0) valueField = fields[numIdx]
            }
            if (timestampField == null || valueField == null) {
                // skip rows where either timestamp or value can't be located
                android.util.Log.d("IPERLGoogleSheetsReader", "[WaterMeter] Skipping row (no timestamp/value): $line")
                continue
            }
            val meterValueNullable = parseFloatNullable(valueField)
            if (meterValueNullable == null) {
                // couldn't parse numeric value - skip this row
                android.util.Log.d("IPERLGoogleSheetsReader", "[WaterMeter] Skipping row (non-numeric value): $line")
                continue
            }
            val date = parseDateFlexible(timestampField, dateFormatCandidates)
            if (date != null) {
                readings.add(WaterMeterReading(timestampField, meterValueNullable, date))
            } else {
                android.util.Log.d("IPERLGoogleSheetsReader", "[WaterMeter] Skipping row (invalid date): $line")
            }
        }
